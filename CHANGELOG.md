...
- **Empty transcript with nothing to explain it.** The background IQ collector takes the dongle on a rotation, but it did so by calling `input_source.stop()` directly — bypassing the arbiter, while the release path handed the radio back *through* it. The console therefore reported `LOCKED` on the operator's preset while the radio was off collecting on another band: dead audio, and a transcript quietly filling with filtered `[inaudible]`. The collector now reports through `adopt()`, and the Listen view names who has the radio and how to keep it.
